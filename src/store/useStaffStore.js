...
export const useStaffStore = create((set, get) => ({
  ...
  assignMachine: (workerId, machineId) => {
    ...
    const m = machineId ? machineById(machineId) : null;
    let status;
    if (machineId) {
    ...
    get().patchWorker(workerId, {
      ...
      assignedMachine: m ? m.name : '—',
      assignedMachineCode: m ? m.code : '—',
      status,
    });
  },

  assignWorkflow: (workerId, workflowId) => {
    const wf = WORKFLOW_OPTIONS.find((x) => x.id === workflowId);
    get().patchWorker(workerId, {
      workflowId,
      assignedWorkflow: wf?.label ?? '—',
    });
  ...
  assignShift: (workerId, shiftId) => {
    const sh = SHIFT_OPTIONS.find((s) => s.id === shiftId);
    get().patchWorker(workerId, {
      shiftId,
      shiftLabel: sh?.label ?? '—',
    });
